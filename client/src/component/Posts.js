  ...
  ShowComment() {
    if (this.props.postDetails.comments.length == 0)
      ...
      var coments = this.props.postDetails.comments.map((each) => {
        ...
        var options = null;
        if (eachcomment.author.id == this.props.Auth.user._id) {
          options = (
            <Nav>
              <NavDropdown
                title=''
                style={{ paddingLeft: 'auto', marginTop: '9px' }}
              >
    ...
    if (this.state.show == true) {
      return (
        <div style={{ paddingTop: '4px' }} show={this.state.show}>
          <div style={{ backgroundColor: '#ebedeb' }}>
            <h4 style={{ paddingLeft: '35%' }}>Comments</h4>
            {coments}
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <div className='row' style={{ marginBottom: '30px', width: '100%' }}>
            ...
            <div className='col-8'>
              <h2 style={{ marginTop: '10px', marginLeft: '25px' }}>
                {this.props.postDetails.author.username}
              </h2>
            </div>
            <div className='col-1 '>{option}</div>
          </div>
          <h4>{this.props.postDetails.description}</h4>
          <Image src={this.props.postDetails.image} fluid />
          <Navbar style={{ marginBottom: '15px', padding: '0' }}>
          ...
          >
            <Media>
              ...
              <Media.Body>
                <div className='row'>
                  <div className='col-8'>
                    <h3>{this.props.postDetails.author.username}</h3>
                  </div>
                  <div className='col-1 offset-3'>{option}</div>
                </div>
              </Media.Body>
            </Media>
